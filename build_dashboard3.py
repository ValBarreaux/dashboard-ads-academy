...
# ---------- ROAS (VSL tracking sheet : ventes issues des pubs) ----------
import os as _os
sales={"adspend":0,"ca_signe":0,"cash":0,"nventes":0,"roas_signe":0,"roas_cash":0,"by_month":{}}
if _os.path.exists('sales_roas.json'):
    try: sales=json.load(open('sales_roas.json'))
    except Exception: pass
r_ad=sales.get('adspend',0); r_ca=sales.get('ca_signe',0); r_cash=sales.get('cash',0)
r_nv=sales.get('nventes',0); r_rs=sales.get('roas_signe',0); r_rc=sales.get('roas_cash',0)
_rcls=lambda v:'good' if v>=2 else ('mid' if v>=1 else 'bad')
bym=sales.get('by_month',{})
if bym:
    _mr=''.join(f"<tr><td>{k}</td><td class='r'>{num(bym[k].get('adspend',0))} \u20ac</td><td class='r strong'>{num(bym[k].get('ca',0))} \u20ac</td><td class='r'>{num(bym[k].get('cash',0))} \u20ac</td></tr>" for k in sorted(bym))
    roas_month_html=f"<h3>D\u00e9tail par mois</h3><table><thead><tr><th>Mois</th><th class='r'>D\u00e9pense</th><th class='r'>CA sign\u00e9</th><th class='r'>Encaiss\u00e9</th></tr></thead><tbody>{_mr}</tbody></table>"
else:
    roas_month_html="<p class='ptxt'>En attente des donn\u00e9es de la VSL tracking sheet.</p>"
roas_section=f"""<h2 class="s1">\U0001f4b0 ROAS \u2014 ventes issues des pubs (VSL tracking)</h2>
<div class="kpis">
<div class="kpi"><div class="lab">D\u00e9pense pub</div><div class="val">{num(r_ad)} \u20ac</div></div>
<div class="kpi"><div class="lab">CA sign\u00e9 (ads)</div><div class="val">{num(r_ca)} \u20ac{f' <small>{r_nv} ventes</small>' if r_nv else ''}</div></div>
<div class="kpi"><div class="lab">Encaiss\u00e9 \u00e0 ce jour</div><div class="val">{num(r_cash)} \u20ac</div></div>
<div class="kpi"><div class="lab">ROAS sign\u00e9</div><div class="val {_rcls(r_rs)}">{r_rs:.2f}x</div></div>
<div class="kpi"><div class="lab">ROAS encaiss\u00e9</div><div class="val {_rcls(r_rc)}">{r_rc:.2f}x</div></div>
</div>
<div class="note gold"><b>Source : ta VSL tracking sheet</b> (suivi quotidien, uniquement les ventes issues des pubs, tous closers confondus). <b>ROAS sign\u00e9</b> = CA contractualis\u00e9 \u00f7 d\u00e9pense pub ; <b>ROAS encaiss\u00e9</b> = cash d\u00e9j\u00e0 encaiss\u00e9 \u00f7 d\u00e9pense (il monte \u00e0 mesure que les Split-Pay se paient).</div>
<div class="panel" style="margin-top:14px">{roas_month_html}</div>"""
...
<div class="panel"><h3>Couche ventes / ROAS</h3><p class="ptxt">La couche ventes est branchée sur ta <b>VSL tracking sheet</b> (suivi quotidien, uniquement les ventes issues des pubs, tous closers). Voir la section <b>ROAS</b> juste en dessous.</p></div>
</div>
...
print("written",len(H),"bytes . ROAS",r_rs,"x . CA",r_ca,"EUR")
